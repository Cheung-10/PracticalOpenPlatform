      ...
      <el-col :xs="24" :sm="24" :md="12" :lg="8" :xl="8">
        ...
        <el-form-item label="模型选择">
          ...
          <el-form-item label="模型名称">
            <el-input v-model="Form.deployTask.name"></el-input>
          </el-form-item>
          <el-form-item label="模型描述">
            <el-input v-model="Form.deployTask.description"></el-input>
          </el-form-item>
...
<script>
  export default {
    created(){
      this.$get2('trainModelTemplate/getAll').then(res =>{
          alert('请在选择模版后参照示例填写参数')
          this.modelType = res.data.data;
          this.trainModelTemplateId = this.modelType[modelTypeIndex].id
        })
    },
    name: 'Modelt',
    data() {
    ...
    methods: {
      ...
      getAllByCurrentUser(){
        this.$get2('/trainModelTask/getAllByCurrentUser').then(res =>{
          this.taskId = res.data.data
          this.dialogFormVisible5 = true
        })
      },
      submitExecuteForm(){
        this.$post2('/trainModelTask/execute',this.Form).then(res =>{
          if(res.data.status===200)
          {this.$message({
              message: '成功',
              type: 'success',
      ...
      stopById(row) {
        this.$post2('/trainModelTask/stop',{
          taskId : row.id
        }).then(res=>{
          if(res.data.status==='200'){
            this.$message({
